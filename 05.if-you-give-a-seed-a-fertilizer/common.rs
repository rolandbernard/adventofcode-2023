...
impl Ranges {
    ...
    pub fn from_iter<I: Iterator<Item = (usize, usize)>>(iter: I) -> Self {
...
impl Mapping {
    pub fn new(mut ranges: Vec<(usize, usize, usize)>) -> Self {
        ranges.sort_by_key(|c| c.1);
        Mapping { mappings: ranges }
    }

    pub fn map(&self, mut inp: Ranges) -> Ranges {
...
pub fn parse_input() -> (Vec<usize>, HashMap<String, (String, Mapping)>) {
...
pub fn min_location_for(
    mut whats_needed: Ranges,
    ...
    return whats_needed.limits().unwrap().0;
}
